...
interface WhoWeServeRecord {
  id: string;
  description: string;
  points: string[];
}

export default function WhoWeServeManagement() {
  ...
  const fetchRecord = async () => {
    try {
      ...
      if (json.success) {
        const first: WhoWeServeRecord | undefined = (json.data || [])[0];
        if (first?.id) {
      ...
      console.error('Error fetching WhoWeServe records:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = { description, points: cleanedPoints };

      const res = await fetch(recordId ? `/api/whoweserve/${recordId}` : '/api/whoweserve', {
      ...
      alert('Who We Serve section saved successfully!');
    } catch (e) {
      console.error('Error saving WhoWeServe record:', e);
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/whoweserve/${recordId}`, { method: 'DELETE' });
      const json = await res.json();
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Who We Serve</h1>
          <p className="text-gray-600">Manage the description and specific points of who you serve.</p>
        </div>
            ...
            {points.map((point, index) => (
              <div key={index} className="flex items-center gap-3">
                ...
                <input
                  ...
                  placeholder="e.g. Humanitarian Organizations"
                />
                <button
